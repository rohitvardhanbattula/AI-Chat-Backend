...
service AIService {
    ...
    entity ChatSessions as projection on db.ChatSessions
        excluding { functionalspec };          // never expose full spec in list queries

    entity ChatMessages as projection on db.ChatMessages;

    entity Users as projection on db.Users
        excluding { passwordHash, otp, otpExpiry, failedLogins, lockedUntil };
    ...
    // ── Chat actions ──────────────────────────────────────────────────────────
    action generateMultiModelResponse(
        prompt: String, category: String, extractedText: LargeString
    ) returns array of ModelResponse;

    action sendChatMessage(
        sessionId: UUID, modelId: String, prompt: String,
        category: String, extractedText: LargeString
    ) returns LargeString;

    // ── Utility actions ───────────────────────────────────────────────────────
    action submitRating(
        userId: String, modelId: String, category: String, rating: Integer
    ) returns String;

    action validateABAPCode(code: LargeString) returns array of String;

    action establishConnection(
        sessionId: String, url: String, user: String,
        password: String, client: String, language: String
    ) returns String;
}
